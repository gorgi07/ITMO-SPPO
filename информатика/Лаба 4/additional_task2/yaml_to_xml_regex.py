...
class YamlToXmlParserRegex:
    ...
    def __yaml_str_editing(self, yaml_file) -> list:
        ...
        for i in range(len(yaml_doc)):
            if re.match(r"^\s*-\s\w*.*", yaml_doc[i]):
    ...
    def __yaml_to_xml(self) -> str:
        ...
        for i in range(len(self.__yaml_doc)):
            ...
            if indent < last_indent:
                for j in range(len(keys) - 1, -1, -1):
                    if keys[j] < last_indent:
                        if re.match(r"^\s*-\s\w*.*", self.__yaml_doc[i - 1]):
            ...
            if ((re.match(r"^\s*\w*:\s*$", element) and i != len(self.__yaml_doc) - 1 and
                 (not (re.match(r"^\s*-\s\w*.*", self.__yaml_doc[i + 1])) or re.match(r"^.*:.*$", self.__yaml_doc[i + 1]))) or
                    (re.match(r"^\s*\w+:\s*$", element) and i == len(self.__yaml_doc) - 1)):
            ...
            # то записываем тэг
            elif (re.match(r"^\s*\w+:\s*$", element) and i != len(self.__yaml_doc) - 1
                  and re.match(r"^\s*-\s\w*.*", self.__yaml_doc[i + 1])):
                content = element.replace(":", "").replace("-", "").strip()
                last_tags[indent] = content
            ...
            elif re.match(r"^\s*-\s\w+:\s\w*\s*$", element):
            ...
            elif re.match(r"^\s*\w+:\s\w*\s*$", element):
                content = [i.replace("-", "").strip() for i in element.split(": ")]
                out += "\t" * indent + f"<{content[0]}>{content[1]}</{content[0]}>\n"
                last_tags[indent] = content[0]
...
if __name__ == '__main__':
    try:
        parser = YamlToXmlParserRegex("C:/Users/erokh/PycharmProjects/Lab4/data/Input.yaml")
        parser.convert_to_xml(new_file=True)
